					...
					<td class="price-filter" style="text-align: left">
						<form action="<c:url value="/product/list"/>" method="get">
							<input type="hidden" value="${TName }" name="tName"> <input
								...
								value="${pm.cri.minPrice }"> <span class="mx-[6px]"><svg
				...
				<tr>
					...
					<td>
						<ul class="flex undefined">
							<li class="mr-5"><label for="saleYn"
								class="flex items-center justify-start text-base font-medium break-all cursor-pointer text-jnBlack">
									<svg width="20" height="20" viewBox="2 2 20 20" fill="#C2C6CE"
										xmlns="http://www.w3.org/2000/svg"
...
<path
											...
											stroke-linejoin="round"></path>
<path d="M16 9L10.5 14.5L8 12" stroke="#FFFFFF" stroke-width="1.5"
											stroke-linecap="round" stroke-linejoin="round"></path>
</svg> <span class="text-base ps-1">모든 상품 보기</span>
							</label> <label for="saleZn"
								class="flex items-center justify-start text-base font-medium break-all cursor-pointer text-jnBlack">
									<svg width="20" height="20" viewBox="2 2 20 20" fill="#C2C6CE"
										xmlns="http://www.w3.org/2000/svg"
										class="mr-1 pointer-events-none">
...
<path d="M16 9L10.5 14.5L8 12" stroke="#FFFFFF" stroke-width="1.5"
											stroke-linecap="round" stroke-linejoin="round"></path>
</svg> <span class="text-base ps-1">판매완료 상품 제거</span>
							</label> <input id="saleYn" name="state" class="hidden" type="radio"
								value="all"
								<c:if test="${pm.cri.state == 'all' }">checked</c:if>> <input
								id="saleZn" name="state" class="hidden" type="radio"
								value="except"
								<c:if test="${pm.cri.state == 'except' }">checked</c:if>>
							</li>
						</ul>
					</td>
				</tr>
		...
		<select class="form-control col-4 offset-8 mb-4" name="order">
			<option value="pr_num"
				<c:if test="${pm.cri.order == 'pr_num' }">selected</c:if>>최신순</option>
			<option value="pr_view"
				<c:if test="${pm.cri.order == 'pr_view' }">selected</c:if>>조회수순</option>
			<option value="pr_basket"
				<c:if test="${pm.cri.order == 'pr_basket' }">selected</c:if>>찜순</option>
			<option value="desc"
				<c:if test="${pm.cri.order == 'desc' }">selected</c:if>>가격
				높은순</option>
			<option value="asc"
				<c:if test="${pm.cri.order == 'asc' }">selected</c:if>>가격
				낮은순</option>
		</select>
			...
			<c:forEach var="pro" items="${pList}" varStatus="loop">
				<div class="product-container">
                    ...
                    <text x="50%" y="50%" alignment-baseline="middle"
											text-anchor="middle" fill="white" font-size="10">판매완료</text>
                  </svg>
								</c:when>
                    ...
                    <rect x="0" y="0" width="40" height="20" rx="4"
											fill="#0DCC5A"></rect>
                    <text x="50%" y="50%" alignment-baseline="middle"
											text-anchor="middle" fill="white" font-size="12">예약중</text>
                  </svg>
								</c:when>
							</c:choose>
						</p>
