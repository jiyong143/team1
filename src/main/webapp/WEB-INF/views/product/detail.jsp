	...
	<div class="container">
		...
		<div class="infoContainer">
				...
				<div>
					...
					<c:choose>
						...
						<c:when test="${info.pr_price == -10}">
							<h2 style="font-size: 20px; color: #808080; font-weight: bold;">가격
								제안</h2>
						</c:when>
						<c:otherwise>
				...
				<c:choose>
					<c:when test="${!empty user}">
						<c:if test="${loginUser.me_id ne prUser.me_id}">
							...
							<a
								href="<c:url value="/report/insertProduct?rePrNum=${info.pr_num}"/>"
								class="btn btn-outline-success btn-report">신고하기</a>
							<c:if test="${info.pr_buyId == loginUser.me_id}">
								<button class="btn btn-outline-success btn-liquidate">포인트로
									결제하기</button>
							</c:if>
						</c:if>
					</c:when>
			...
			<c:if test="${loginUser.me_id eq prUser.me_id}">
				<ul class="jiyong-ul flex w-full py-3 rounded">
					<c:if test="${info.pr_ps_state ne '판매완료'  }">
						<!-- db에서 해당 상품의 시간을 현재로 수정 -->
						<li
							class="jiyong-li flex flex-1 basis-[25%] items-center justify-center px-3 relative after:absolute [&amp;:not(:first-child)]:after:w-[1px] after:bg-gray-300 after:h-9 after:left-0 [&amp;:not(:first-child)]:after:content-['']">
							<form action="<c:url value="/product/up?num=${info.pr_num }"/>"
								method="post">
								<button
									class="jiyong-button flex flex-col items-center py-[6px]">
									...
									<p class="mt-1 text-xs font-medium text-center text-jnBlack">위로
										올리기</p>
								</button>
							</form>
						</li>
						...
						<li
							class="jiyong-li flex flex-1 basis-[25%] items-center justify-center px-3 relative after:absolute [&amp;:not(:first-child)]:after:w-[1px] after:bg-gray-300 after:h-9 after:left-0 [&amp;:not(:first-child)]:after:content-['']">
							<a href="<c:url value="/product/update?num=${info.pr_num }"/>"
							method="get">
								<button
									class="jiyong-button flex flex-col items-center py-[6px]">
									<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
					...
					<li
						class="jiyong-li flex flex-1 basis-[25%] items-center justify-center px-3 relative after:absolute [&amp;:not(:first-child)]:after:w-[1px] after:bg-gray-300 after:h-9 after:left-0 [&amp;:not(:first-child)]:after:content-['']">
						<button
							class="jiyong-button flex flex-col items-center py-[6px] deleteBtn">
							<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
								xmlns="http://www.w3.org/2000/svg">
		...
		<div class="infoBox">
			...
			<textarea id="product-content" name="product-content"
				...
				data-gtm-form-interact-field-id="0" readonly>${info.content }</textarea>
		</div>
		<div class="sellerContainer">
			<a class="sellerA"
				href="<c:url value='/member/mypage?me_id=${prUser.me_id}'/>"> <!-- 판매자 신상 -->
				<h3>판매자</h3>
				<hr>
				<h2>${prUser.me_id}</h2>
				<div>
						...
						<table class="w-100">
							...
							<tbody>
								<tr>
									...
									<!-- 거래횟수 -->
									<td style="text-align: center;">${reviewNum}</td>
									<!-- 거래후기갯수 -->
								</tr>
							</tbody>
						</table>
